    ...
    <li className="rounded-2xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-xs text-neutral-500">{provider.base_url}</p>
...
export function ProvidersPage() {
  ...
  return (
    <SidebarLayout title="Provedores de IA" sidebar={<AppSidebar current="providers" />}>
      <div className="min-h-0 flex-1 overflow-y-auto bg-neutral-50 dark:bg-neutral-900">
        <div className="mx-auto w-full max-w-6xl px-6 pb-14 pt-10 lg:px-12">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500 dark:text-neutral-400">
            Modelos de outros provedores usados pelo roteador, ao lado dos modelos
            locais e do Ollama Cloud. As chaves ficam protegidas no seu usuário do
            sistema e nunca voltam para esta tela.
          </p>
          {!loading && !error && (
          ...
          {loading && <p className="mt-8 text-sm text-neutral-500">Carregando provedores…</p>}
          {!loading && !error && providers.length === 0 && (
            <p className="mt-8 text-sm text-neutral-500">
              Nenhum provedor configurado. Defina OLLAMA_DZ23_CONFIG com um arquivo de
              provedores.
            </p>
          )}
