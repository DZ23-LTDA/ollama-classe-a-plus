...
export default function AgenticConsole() {
  ...
  const createMission = async () => {
    ...
    try {
      const created = await api<Mission>("/api/agent/v1/missions", { method: "POST", body: JSON.stringify({ objective, provider, model: providerModels[provider], project_id: projectID || undefined, auto_run: false }) });
  ...
  const decide = async (approval: { id: string; nonce?: string }, approved: boolean) => {
    ...
    setBusy(true);
    try {
      await api(`/api/agent/v1/missions/${encodeURIComponent(mission.id)}/approvals/${encodeURIComponent(approval.id)}`, { method: "POST", body: JSON.stringify({ approved, nonce: approval.nonce, reason: approved ? "Aprovado no Agentic Console" : "Rejeitado no Agentic Console" }) });
