...
| Skills/MCP | `VALIDADA LOCALMENTE`: manifests não confiáveis por padrão, scopes, allowlists, MCP stdio/Remote MCP, enable/disable/remove e aprovação por ferramenta | Instalação, atualização, isolamento, lifecycle e aprovação por ferramenta | Teste com servidores MCP externos, OAuth/pareamento e matriz de permissões por tenant |
...
| Grok Live | `VALIDADA LOCALMENTE`: cliente Responses com retry, circuito, streaming, status sanitizado e distinção entre fontes ao vivo/memória | Pesquisa e raciocínio com provider xAI, citações e tools dentro de missões | Chave/quota xAI, web search/Voice/Imagine e smoke externo autorizado |
| Agentes departamentais | `VALIDADA LOCALMENTE`: agentes persistentes por departamento, orçamento, supervisor, memória, SLA, pausa/retomada e escopos | Operação contínua de empresa com avaliação e supervisão humana | Workers distribuídos, conectores reais, avaliação longitudinal e staging |
| Evaluation OS e routing | `VALIDADA LOCALMENTE`: casos determinísticos de coding, browser, tools, segurança, memória, planejamento e router por capacidade/saúde/custo | Seleção adaptativa de provider e regressão contínua por jornada | Dataset real, métricas históricas, judge autorizado e calibração por domínio |
